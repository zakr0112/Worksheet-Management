...
uses
  ...
  FMX.ListView.Types, FMX.ListView.Appearances, FMX.ListView.Adapters.Base, System.Skia, FMX.Memo.Types,
  FMX.ScrollBox, FMX.Memo, FMX.Edit, FMX.Layouts, FMX.MultiView, FMX.Skia, FMX.ListView, FMX.TabControl,
  ...
  {$ENDIF}
  FMX.Objects, FMX.DateTimeCtrls, FMX.ListBox, FMX.DialogService, FMX.DialogService.Async,
  FMX.ComboEdit, FMX.MediaLibrary, System.Actions, FMX.ActnList, FMX.StdActns, FMX.MediaLibrary.Actions,
  ...
  uDataModule, uCommonDialogs, uCommon, uCustomerManagerClass, uHelperTabControl, uJobsManagerClass,
  uReportAndExportHelper;
...
begin
  // Changed 16.04.2026 If Assigned not required, object.free checks assigned automatically
  Job.Free;
  Customer.Free;
end;

function TJobForm.GetSeconds(const ATime: TDateTime): Integer;
var
...
begin
  ...
  begin
    ...
    if ItemObject.Name = 'lvoImgHtml' then
    begin
      if not Job.FetchJob(jobno) then
      begin
        ShowWarning('Unable to find the selected record!');
      ...
      ShowWorksheetHTMLThread(jobno, Job, aniProgress, rectProgress);
      exit;
    end;

    if ItemObject.Name = 'lvoImgEdit' then
    begin
      ClearJobRecord;
      if not Job.FetchJob(jobno) then
      begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        Data['lvoImgHtml'] := imgHtml.Bitmap; // added for html icon display
      end;
      qryListjobs.Next;
    end;
  finally
...
procedure TJobForm.RequestPermissions();
begin
...
begin
  ...
  try
    ...
    // replaced with params to help prevent sql injection
    qryJobphotos.SQL.Text := 'SELECT photo, photono FROM jobs_photos WHERE jobno = :jobno ORDER BY photono LIMIT 6';
    qryJobphotos.ParamByName('jobno').AsInteger := job.Jobno;
    qryJobphotos.Open;
