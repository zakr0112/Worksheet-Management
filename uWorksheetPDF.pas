...
uses
  ...
  {$IFDEF Android}
    Posix.Unistd,
  {$ENDIF}
  FMX.Graphics,FMX.Skia, Skia,
  uCommon, uDatamodule, uCommonDialogs, uJobsManagerClass, uCommonUTF8Helper, uReportAndExportHelper;

type
  TWorksheetPDF = class(TFPDFExt)
  protected
  public
    fJobno: integer;
    fJobdate: string;
...
// Code via copilot
function ExportSignatureAsPNG(const ASvgSource: string; const ASignatureFilename: string): Boolean;
var
  LBitmap: TBitmap;
begin
  ...
  try
    try
      ...
      Result := FileExists(ASignatureFilename);
    except
      Result := false;
    end;
  finally
...
procedure TWorksheetPDF.Footer;
begin
  inherited;
...
procedure TWorksheetPDF.ForcePageIfNoRoom(const ARequiredheightmm: integer);
begin
...
procedure TWorksheetPDF.Header();
begin
  inherited;
  SetFillColor(244, 244, 244);
  SetDrawColor(124, 124, 124);
  ...
  var LOGO :=
    {$IF DEFINED(ANDROID)}
    TPath.Combine(TPath.GetDocumentsPath, 'logo.png'); // logo is deployed to app base path (e.g. .\assets\internal)
    {$ELSE}
    Tpath.Combine(TPath.GetDirectoryName(ParamStr(0)), 'logo.png'); // deployed to application exe folder
    {$ENDIF}
  if FileExists(LOGO) then
    Image(LOGO, 10, 8, 20, 20);
  // Center the header info (company, taglines etc) in the header center
  MultiCell(190, 6, FHeadertitle, '0', 'C', false);
  Ln(5);
  // Now print the job number in the right hand side top of page
  SetY(10.1);
  SetX(-40.0);
  SetFont('Times', '', 18);
...
function TWorksheetPDF.PrintJob(): boolean;
var
...
begin
  ...
  try
    try
      ...
      on E: Exception do
      begin
        ShowException('PDFMainSection', E);
        exit(false);
      end;
    end;
  finally
  ...
  try
    try
      ...
      begin
        ...
        begin
          var SignatureFilename := TPath.Combine(COMMON_FILE_PATH, 'signature.png');
          if ExportSignatureAsPNG(Job.GetSignatureSVG, SignatureFilename) then
          begin
  ...
  try
    try
      // We delete any exisitng photo files
      DeleteFilesWithSuffix(COMMON_FILE_PATH, 'photo_');

      var photofile: string;
      DM.qryPDF.Close;
      I := 1;  // Print 2 per page, so determines photo count as 1 3 5 require a new page
      ...
      begin
        ...
        begin
          try
            photofile := TPath.Combine(COMMON_FILE_PATH, Format('photo_%d.png', [DM.qryPDF.FieldByName('photono').AsInteger]));
            ...
            ShowToast('whoops'); // should never get here!
          end;
        end;
        DM.qryPDF.Next;
      end;
...
procedure TWorksheetPDF.SetDataFont();
begin
  SetFont('Arial', 'B', 10);
...
procedure TWorksheetPDF.SetHeaderSection(const ASectionTitle: string);
begin
  SetFont('Times','B', 12);
...
procedure TWorksheetPDF.SetTheMetaData;
begin
  self.SetSubject('WORKSHEET PRIVATE AND CONFIDENTIAL');
  self.SetUTF8(true);
  self.SetAliasNbPages();
  self.SetCreator('WorksheetsZR');
  self.SetAuthor('WorksheetsZR');
end;

procedure TWorksheetPDF.SetTitleFont();
begin
  SetFont('Times', '', 9);
  SetFillColor(255); // white
  SetDrawColor(136);
