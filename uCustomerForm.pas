...
begin
  Result := false;
  try
    qryCustomerDelete.close;
    qryCustomerDelete.ParamByName('custid').AsInteger := Acustid;
    qryCustomerDelete.ExecSQL();
    if qryCustomerDelete.RowsAffected < 1 then
      exit;
    Result := true;
  except
    on E: exception do
